...
{
    private static final Logger log = LogProxy.getLogger(PageInterceptor.class);
    ...
    private String getOraclePageSql(PageParam<?> pageParam, String sql)
    {
        ...
        return "select * from (select rownum rn, t.* from (" + sql + ") table1 where rownum <= " + (endrow)
    ...
    private String getSqlServerPageSql(PageParam<?> pageParam, String sql)
    {
        ...
        // 命名为table1、table2是为了避免与入参中的sql 别名冲突
        return "select top " + pageSize + " from (" + sql + ") table1 where table1.id not in (select top " + offset
