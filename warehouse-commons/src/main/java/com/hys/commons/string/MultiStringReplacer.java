...
public final class MultiStringReplacer
{
    ...
    private final Map<Character, MultiStringReplacer> childs = new HashMap<Character, MultiStringReplacer>();
    ...
    /*
     * private FindResultPosInfo compareWords(String s, int idx) { if(this.oldWord != null) return new
     * FindResultPosInfo(this.oldWord, this.newWord, idx - this.oldWord.length()); if(idx >= s.length()) return null;
     * char c = s.charAt(idx); MultiStringReplacer ti = childs.get(c); if(ti == null) return null; return
     * ti.compareWords(s, idx + 1); }
     */
