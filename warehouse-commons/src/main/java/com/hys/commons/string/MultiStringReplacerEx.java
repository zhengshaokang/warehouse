...
/**
 * 用于替换多个字符串的类，与MultiStringReplacer不同的是它会忽略非中文字符 如：关键字里面有“操比”，那么同时也会过滤“操x比”之类的词
 * 
 */
public final class MultiStringReplacerEx
{
    // 子节点列表，Map的key是子节点对应的字符
    private final Map<Character, MultiStringReplacerEx> childs = new HashMap<Character, MultiStringReplacerEx>();
    ...
    {
        return !MultiStringReplacerEx.isChineseCharacter(c) && c != '<' && c != '>';
    }
    ...
    private FindResultPosInfo compareWords(String s, int idx, int ignoreCount, int deep)
    {
        ...
        // 查找匹配的字节点
        char c = s.charAt(idx);
        if (deep > 0 && MultiStringReplacerEx.isIgnoreCharacter(c))
        {
            // if(deep > 0){
            int i = 0;
            for (; MultiStringReplacerEx.isIgnoreCharacter(c) && idx + i < s.length(); ++i)
            {
                c = s.charAt(idx + i);
            }
            if (i > 0)
